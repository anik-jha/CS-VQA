...
def main(params):
    ...
    if params['split'] == 1:
        ...
        #subtype = 'coco_train_hadamard_0_25'     #phiTphiX
        subtype= 'train_coco025_blpTp'
        #subtype = 'train_coco025_reconet'  #reconNet  
        #other one for block-based
        subtype2 = 'train2014'
        for i in range(len(train_anno['annotations'])):
            ans = train_anno['annotations'][i]['multiple_choice_answer']
            question_id = train_anno['annotations'][i]['question_id']
            image_path = imdir%(subtype, subtype2, train_anno['annotations'][i]['image_id'])
        ...
        #subtype = 'coco_val_hadamard_0_25'
        subtype= 'val_coco025_blpTp'
        #subtype = 'val_coco025_reconet'  #reconNet  
        subtype2 = 'val2014'
        for i in range(len(val_anno['annotations'])):
            ans = val_anno['annotations'][i]['multiple_choice_answer']
            question_id = val_anno['annotations'][i]['question_id']
            image_path = imdir%(subtype, subtype2, val_anno['annotations'][i]['image_id'])
    ...
    #json.dump(train, open('pTp_vqa_raw_train.json', 'w')) #pTp
    #json.dump(test, open('pTp_vqa_raw_test.json', 'w'))
    json.dump(train, open('blpTp_vqa_raw_train.json', 'w')) #Block based
    json.dump(test, open('blpTp_vqa_raw_test.json', 'w'))
    #json.dump(train, open('recon_vqa_raw_train.json', 'w')) #reconNet
    #json.dump(test, open('recon_vqa_raw_test.json', 'w'))
    #----------------------output file name-----------------------------------
...
if __name__ == "__main__":
    ...
    # input json
    parser.add_argument('--download', default='True', help='Download and extract data from VQA server')
