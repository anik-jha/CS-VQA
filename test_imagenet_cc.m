...
caffe.set_device(0);
%imagenet root
test_images_root= '/media/twotb/imagenet/ILSVRC2012_img_train';
test_dirs=dir(test_images_root);
%test_dirs=test_dirs(4:end);
test_dirs(1:3) = [];
fileID = fopen('output.txt','w');
fileID_err = fopen('error.txt','w');
c=makecform('cmyk2srgb');


try
    modify_prototxt(prototxt_file, 64);
    net = caffe.Net(prototxt_file, caffemodel, 'test');
catch
    %warning('size error %s at size %f',fullfile(test_images_root,test_images_dir,image_name),size(input_im_nn));
    disp('some issue here')
end

for testdir = 937:length(test_dirs)
%    test_dirs(testdir).name
%end
% Dataset for testing
%test_images_dir = './test_images';
test_images_dir = test_dirs(testdir).name;
test_images = dir(fullfile(strcat(test_images_root,'/',test_images_dir),'*.JPEG'));
%test_images = test_images(3:end);

%output_dir = ['./reconstruction_results/mr_0_', mr_str, '/'];
output_dir = ['./reconstruction_results/mr_0_', mr_str, '/', test_images_dir, '/'];
mkdir(output_dir)
...
for image_number = 1:length(test_images)
	...
	start_time = tic;
        ...
        im = im2double(imread(fullfile(test_images_root,test_images_dir,image_name))); %Input for the ReconNet
        if (size(im,3)<3)
            im=cat(3,im,im,im);
        end
        if(size(im,3)>3)
        ...
        subfolder = fullfile(test_images_root,test_images_dir,image_name);
        %fprintf(subfolder)
